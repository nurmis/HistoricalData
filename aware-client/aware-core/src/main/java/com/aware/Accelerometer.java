...
import com.aware.simulator.ContextDataReading;
...
public class Accelerometer extends Aware_Sensor implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        rowData.put(Accelerometer_Data.DEVICE_ID, "testPhone");
        ...
        Log.d("Tester", "after contentvalues");
        data_values.toArray(data_buffer);
        Log.d("Tester", "after data_values.toArray");

        try {
            Log.d("Tester", "inside try");
            if (!Aware.getSetting(appC, Aware_Preferences.DEBUG_DB_SLOW).equals("true")) {
                Log.d("Tester", "before execute");
    ...
    private class AsyncStore extends AsyncTask<ContentValues[], Void, Void> {
        ...
        protected Void doInBackground(ContentValues[]... data) {
            appC.getContentResolver().bulkInsert(Accelerometer_Data.CONTENT_URI, data[0]);
            return null;
        }
    }
    ...
    private void saveAccelerometerDevice(Sensor acc) {
        ...
        Log.d("Tester", "accelInfo:" + accelInfo);
        accelInfo = null;
        if( accelInfo == null || ! accelInfo.moveToFirst() ) {
            ...
            rowData.put(Accelerometer_Sensor.DEVICE_ID, "testPhone");
            ...
            Log.d("Tester", "rowdata:" + rowData.toString());
    ...
    public void onCreate() {
        ...
        filter.addAction(ACTION_AWARE_ACCELEROMETER);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("Tester", "Accelerometer onStartCommand");
        TAG = Aware.getSetting(getApplicationContext(), Aware_Preferences.DEBUG_TAG).length() > 0 ? Aware.getSetting(getApplicationContext(), Aware_Preferences.DEBUG_TAG) : TAG;
        ...
        if (SAMPLING_RATE != Integer.parseInt(Aware.getSetting(getApplicationContext(), Aware_Preferences.FREQUENCY_ACCELEROMETER))) { //changed parameters
            ...
            sensorHandler.removeCallbacksAndMessages(null);
            //mSensorManager.unregisterListener(this, mAccelerometer);
            //mSensorManager.registerListener(this, mAccelerometer, 2000000000, sensorHandler);
            }
